...
class PMF(object):
    ...
    def fit(self, train_vec, test_vec):
        # mean subtraction
        self.mean_inv = np.mean(train_vec[:, 2])  # mean value of rating

        pairs_train = train_vec.shape[0]  # train data length
        pairs_test = test_vec.shape[0]  # test data length
        ...
        if ((not incremental) or (self.w_Item is None)):
            # initialize
            self.epoch = 0
            # numpy.random.randn anime feature -> normal
            self.w_Item = 0.1 * np.random.randn(num_item, self.num_feat)
            # numpy.random.randn user feature -> normal  
        ...
        while self.epoch < self.maxepoch:
            ...
            for batch in range(self.num_batches):  
                ...
                if batch == self.num_batches - 1:
                    ...
                    # Print info
                    if batch == self.num_batches - 1:
                        print('The epoch: %f, Training RMSE: %f, Test RMSE %f' % (self.epoch, self.rmse_train[-1], self.rmse_test[-1]))
